...
{id:4,name:'Nova Kids',category:'أطفال',icon:'🧸',url:'https://test-streams.mux.dev/x36xhzz/x36xhzz.m3u8',enabled:true,featured:false,description:'محتوى عائلي آمن وممتع.'},
{id:5,name:'Nova Documentary',category:'وثائقي',icon:'🌍',url:'https://test-streams.mux.dev/x36xhzz/x36xhzz.m3u8',enabled:true,featured:false,description:'قصص وثائقية من حول العالم.'},
{id:6,name:'Nova Sports 2',category:'رياضة',icon:'🏆',url:'https://test-streams.mux.dev/x36xhzz/x36xhzz.m3u8',enabled:true,featured:false,description:'بطولات وتحليلات رياضية.'}
];
...
function normalizeChannel(item){return {id:item.id||Date.now(),name:String(item.name||'').trim(),category:String(item.category||'عام').trim(),icon:String(item.icon||'📺').trim(),url:String(item.url||'').trim(),enabled:item.enabled!==false,featured:item.featured===true,description:String(item.description||'').trim(),logoUrl:String(item.logoUrl||'').trim(),sortOrder:Number(item.sortOrder||0)};}
...
const api={
 ...
 getChannels({includeDisabled=true}={}){const list=read(KEYS.channels,DEFAULT_CHANNELS).map(normalizeChannel).sort((a,b)=>(a.sortOrder||0)-(b.sortOrder||0));return includeDisabled?list:list.filter(x=>x.enabled);},
 ...
 removeChannel(id){const list=this.getChannels().filter(x=>String(x.id)!==String(id));this.saveChannels(list);return true;},
 ...
 exportAll(){return {version:1,provider:'local',channels:this.getChannels(),codes:this.getActivationCodes(),exportedAt:new Date().toISOString()};},
